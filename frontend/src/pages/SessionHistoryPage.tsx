import { ArrowUpRight, FileCheck2, Filter, LoaderCircle, Plus, Trash2 } from "lucide-react";
      ...
      <main className="min-h-screen lg:pl-64">
        ...
        <div className="mx-auto max-w-7xl space-y-8 px-5 py-8 sm:px-8 lg:px-10">
          ...
          <div className="grid gap-5 sm:grid-cols-3">
            <div className="rounded-xl border border-red-500/15 bg-white/[0.035] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07),0_18px_40px_rgba(90,12,28,0.18)] backdrop-blur-xl"><div className="flex items-center gap-2 text-xs uppercase tracking-wider text-slate-500"><FileCheck2 className="h-4 w-4 text-red-400" /> Current records</div><p className="mt-3 text-3xl font-black text-white">{scans.length}</p></div>
            <div className="rounded-xl border border-red-500/20 bg-red-950/10 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07),0_18px_40px_rgba(127,29,29,0.2)] backdrop-blur-xl"><p className="text-xs uppercase tracking-wider text-slate-500">Flagged sessions</p><p className="mt-3 text-3xl font-black text-red-300">{scans.filter((scan) => scan.verdict === "Flagged").length}</p></div>
